...
case class RpnCalculator(stack: List[Op] = Nil) {
  ...
  def push(op: Op): Try[RpnCalculator] = {
    op match {
      ...
      case operation: BinOp => {
        def getValue(something: RpnCalculator): Val = {
          val gottenVal = something.peek()
          gottenVal match {
        ...
        val Part1 = getValue(this)
        var restStack: RpnCalculator = pop()._2
        val Part2 = getValue(restStack)
        restStack = restStack.pop()._2

        val result = operation.eval(Part1, Part2)
  ...
  def peek(): Op = {
    if (stack.isEmpty) {
      throw new NoSuchElementException("Error: A List must contain values!")
    }
    else {
      stack.head
    }
